...
with open('Services.old.astro', 'r', encoding='utf-16') as f:
    old_content = f.read()

cards = re.findall(r'(<!-- 0\d: .*? -->\s*<a .*?</a>)', old_content, re.DOTALL)
mockups = {}
slugs = ['seo', 'meta-ads', 'diseno-web', 'ecommerce', 'conversiones']
for i, card in enumerate(cards):
    mockup = re.search(r'(<!-- MOCKUP: .*?)(?=\s*<div class="relative z-10)', card, re.DOTALL)
    if mockup:
        mockups[slugs[i]] = mockup.group(1).strip()

new_grid = """
    <!-- BENTO GRID DINÁMICO -->
    <div class="grid w-full grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[minmax(300px,auto)]">
      {services.map((s, idx) => (
        <a href={`/${s.slug}`} class={`reveal-up group relative overflow-hidden rounded-3xl border border-black/10 transition duration-500 hover:border-black/20 p-8 lg:p-12 flex flex-col justify-between ${bentoClasses[idx % 5]}`} data-speed="0.03" data-spotlight>
...
pattern = r'<!-- BENTO GRID DINÁMICO -->.*?</div>\s*<!-- CUSTOM ANIMATION STYLE FOR MOCKUPS -->'
